...
- [ ] **[LOW — spotted iter 17] Add `category=` field to corpus `Case()`s so `--category` actually works.** `scripts/auto_test_corpus.py` exposes `--category` as a CLI flag but no `Case()` carries the attribute, so category-based filtering is silently dead. Adding tags (`email`, `hallucination-bait`, `formatting`, etc.) would let CONTRIBUTING document the natural-feeling `--category email` pattern instead of the brittle prefix-substring `--filter EM`. Real change to `auto_test_corpus.py` + CONTRIBUTING update; multi-file → kept its own iter.
