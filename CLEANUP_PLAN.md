...
- [ ] **[LOW — spotted iter 12] Verify CONTRIBUTING.md corpus filter case counts.** Lines 76-82 claim specific counts: `--filter FT` "5 + 2 incidental", `--filter SOLO-NUM` "5", `--category hallucination-bait` "5 (H-prefix)", `--category email` "33". Iter-12 regex sweep got 6/6/3 — close but inconclusive. Needs a focused read of `scripts/auto_test_corpus.py`'s corpus structure to verify properly; update CONTRIBUTING if drifted with corpus growth.
...
| 12 | 2026-05-21 ~15:13 | CONTRIBUTING.md verified — all concrete claims accurate (scripts, tests, CI files, paths); corpus filter counts deferred for a focused read | docs only | `pending` |
