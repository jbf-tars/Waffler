...
function updateHotkeyHint() {
  ...
  if (isWin) {
    if (badge) badge.textContent = 'Ctrl + Win';
    if (label) label.textContent = 'Hold to record';
    if (emptyHint) emptyHint.innerHTML = 'Hold <strong>Ctrl + Win</strong> and speak';
  } else {
...
function hideAuthScreen() {
  ...
  // Restore sidebar + main (showAuthScreen hides them)
  const sidebar = document.querySelector('.sidebar');
  ...
  try {
    ...
    if (result.platform === 'Darwin') {
      ...
      if (result.accessibility_granted) {
        ...
        accessDesc.textContent = 'Needed so Waffler can paste text into other apps';
